...
/**
 * Offline 2-D Fenwick (BIT) that supports:
 *   • point-add  upd(x, y, delta)
 *   • rectangle sum query(x1, x2, y1, y2)  (inclusive ranges, 1-based)
 *
 * Coordinates must satisfy: 1 ≤ x < SZ  and  y ≠ 0.
 * Build cost:   O(M log SZ)     (M = number of updates recorded before init)
 * Query/update: O(log² SZ)
 * Memory:       O(M log SZ)
 */
